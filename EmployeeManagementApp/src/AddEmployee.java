...
public class AddEmployee extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == addButton) {
			...
			if(name.length() > 0 && fname.length() > 0 && dob.length() > 0 && salary.length() > 0 && phone.length() > 0 && designation.length() > 0 && aadhar.length() > 0 && address.length() > 0 && email.length() > 0 && education.length() > 0) {
				try {
					Conn con = new Conn();
					String query = "insert into employee values('"+name+"', '"+fname+"', '"+dob+"', '"+salary+"', '"+address+"', '"+phone+"', '"+email+"', '"+education+"', '"+designation+"', '"+aadhar+"', '"+empId+"', '"+ department +"')";
